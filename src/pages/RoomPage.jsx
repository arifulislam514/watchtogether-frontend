  ...
  <div className="absolute bottom-16 left-4 flex flex-col gap-1 pointer-events-none z-10">
    {messages.slice(-4).map((msg, i) => (
      <div key={i} className="bg-black/60 text-white text-sm px-3 py-1 rounded-lg backdrop-blur-sm">
        <span className="text-violet-400 font-medium">{msg.user_name}: </span>{msg.text}
      </div>
    ))}
  </div>
)
  ...
  <div className="flex items-center justify-between py-2">
    <div className="flex items-center gap-2">
      ...
      <span className="text-sm truncate">
        {member.user_name}
        {member.user === currentUserId && <span className="text-gray-500 text-xs ml-1">(you)</span>}
      </span>
    </div>
    <div className="flex items-center gap-2 shrink-0">
      {member.is_ready ? <CheckCircle size={14} className="text-green-400" /> : <Circle size={14} className="text-gray-600" />}
      {isHost && member.user !== currentUserId && (
        <button onClick={() => onRemove(member.user)} className="text-red-400 hover:text-red-300 text-xs">Remove</button>
      )}
    </div>
  </div>
)
...
const RoomPage = () => {
  ...
  const refreshRoom = useCallback(() =>
    authAxios.get(`/api/rooms/${id}/`).then(res => { setRoom(res.data); return res.data }).catch(() => {}),
  [id])
  ...
  useEffect(() => {
    authAxios.get(`/api/rooms/${id}/`)
      .then(res => {
        ...
        const everyoneReady = res.data.members.length > 0 && res.data.members.every(m => m.is_ready)
  ...
  const handleEvent = useCallback((event) => {
    ...
    switch (event.type) {
      ...
      case 'READY':
        setRoom(prev => prev ? {
          ...prev,
          members: prev.members.map(m => m.user === event.user_id ? { ...m, is_ready: event.is_ready } : m)
        } : prev)
      ...
      case 'SEEK':
        if (event.sender_id === myId) break
        applyRemote(() => { if (videoRef.current) videoRef.current.currentTime = event.timestamp })
        break
      ...
      case 'NETWORK_WAIT':
        ...
        applyRemote(() => { if (videoRef.current && !videoRef.current.paused) videoRef.current.pause() })
        break
      ...
      case 'MEMBER_DISCONNECTED':
        ...
        setPausedById(null)  // anyone can resume
        setRoom(prev => prev ? {
          ...prev,
      ...
      default:
        if (event.type?.startsWith('WEBRTC_')) handleSignalingRef.current?.(event)
    }
  ...
  const removeMember = async (userId) => {
    try {
      ...
      setRoom(prev => prev ? { ...prev, members: prev.members.filter(m => m.user !== userId) } : prev)
    } catch (_err) {}
  }
  ...
  const handleBufferEnd = () => {
    if (networkWaitTimer.current) { clearTimeout(networkWaitTimer.current); networkWaitTimer.current = null }
    if (waiting && waitingFor === 'You') {
      setWaiting(false)
      setWaitingFor('')
  ...
  if (!isMember) return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center text-white">
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-8 w-full max-w-sm text-center">
        ...
        {joinError && (
          <div className="bg-red-500/10 border border-red-500/30 text-red-400 text-sm rounded-lg px-4 py-3 mb-4">{joinError}</div>
        )}
        <input type="password" value={joinPassword} onChange={e => setJoinPassword(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && handleJoin()} placeholder="Room password"
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-gray-900 border-b border-gray-800 shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            ...
            {room?.video_detail && <span className="text-sm text-gray-400 truncate">— {room.video_detail.title}</span>}
          </div>
          <div className="flex items-center gap-2 shrink-0">
            {isHost && (
          ...
          {room?.video_detail?.master_url ? (
            <>
              ...
              {pausedBy && (
                <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-black/80 text-white text-sm px-4 py-2 rounded-full z-10 whitespace-nowrap">
                  ⏸ {pausedBy}{pausedById && pausedById !== String(user?.id) ? ' — waiting for them to resume' : ''}
                </div>
              )}
              ...
              {!allReady && (
                <div className="absolute inset-0 bg-black/70 flex items-center justify-center z-20">
                  <div className="bg-gray-900 rounded-xl px-8 py-6 text-center">
                    ...
                    <p className="text-gray-400 text-sm mb-4">All members must be ready before playback starts</p>
            ...
            <div className="flex items-center justify-center h-full text-gray-600">
              <div className="text-center">
                ...
                {isHost && <Button size="sm" className="mt-3" onClick={loadMyVideos}>Select a Video</Button>}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-3 px-4 py-3 bg-gray-900 border-t border-gray-800 shrink-0 flex-wrap">
          {callActive ? (
            <>
              <button onClick={toggleMute} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${isMuted ? 'bg-gray-800 text-gray-400 hover:bg-gray-700' : 'bg-violet-600 text-white hover:bg-violet-700'}`}>
                {isMuted ? <MicOff size={16} /> : <Mic size={16} />}{isMuted ? 'Unmute' : 'Mute'}
              </button>
              <button onClick={toggleVideo} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${isVideoOn ? 'bg-violet-600 text-white hover:bg-violet-700' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>
                {isVideoOn ? <Video size={16} /> : <VideoOff size={16} />}{isVideoOn ? 'Video On' : 'Video Off'}
              </button>
              <Button size="sm" variant="danger" onClick={stopMedia}><PhoneOff size={14} /> Leave Call</Button>
            </>
          ) : (
            <Button size="sm" onClick={() => startMedia(false)}><Phone size={14} /> Join Voice</Button>
          )}
          {Object.entries(remoteStreams).map(([userId, stream]) => (
            <video key={userId} autoPlay playsInline className="w-24 h-16 rounded-lg object-cover bg-gray-800"
      ...
      <div className="w-72 flex flex-col bg-gray-900 border-l border-gray-800 shrink-0">
        <div className="p-4 border-b border-gray-800">
          ...
          {room?.members?.map(member => (
            <MemberItem key={member.id} member={member} isHost={isHost} onRemove={removeMember} currentUserId={user?.id} />
          ))}
        </div>
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <div className="p-3 border-t border-gray-800 flex gap-2">
            ...
            />
            <button onClick={sendChat} className="p-2 bg-violet-600 hover:bg-violet-700 rounded-lg transition-colors shrink-0">
              <Send size={14} />
            </button>
          </div>
        </div>
      ...
      {selectingVideo && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 w-full max-w-md">
            ...
            {myVideos.length === 0 ? <p className="text-gray-500 text-sm">No ready videos found.</p> : (
                ...
                {myVideos.map(video => (
                  <button key={video.id} onClick={() => selectVideo(video.id)}
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      {video.url_360p && '360p '}{video.url_720p && '720p '}{video.url_1080p && '1080p'}
                    </p>
                  </button>
                ))}
              </div>
            )}
            <Button variant="secondary" fullWidth className="mt-4" onClick={() => setSelectingVideo(false)}>Cancel</Button>
          </div>
        </div>
      )}
    </div>
